...
from topology.partitioning_strategy import (
    Partition,
...
from download.download_progress import RepoProgressEvent
from inference.inference_engine import get_inference_engine, InferenceEngine
...
class Node:
    ...
    def on_node_status(self, request_id, opaque_status):
        try:
            ...
            if DEBUG >= 1:
                print(f"Error on_node_status: {e}")
            if DEBUG >= 1:
                traceback.print_exc()
    ...
    async def process_inference_result(
        ...
        if shard.model_id != "stable-diffusion-2-1-base":
            ...
            is_finished = (
                len(self.buffered_token_output[request_id][0])
                >= self.max_generate_tokens
            )
            if shard.is_last_layer() and not is_finished:
                token = await self.inference_engine.sample(
                    result, temp=self.default_sample_temperature
                )
            ...
            intermediate_result, inference_state = self.handle_stable_diffusion(
                inference_state, result
            )
            forward = result
        if shard.is_last_layer():
            self.trigger_on_token_callbacks(
                request_id, intermediate_result, is_finished
            )
            asyncio.create_task(
                self.broadcast_result(request_id, intermediate_result, is_finished)
            )
    ...
    async def process_prompt(
        ...
        if DEBUG >= 2:
            print(
                f"[{request_id}] process prompt: {base_shard=} {shard=} {prompt=} {elapsed_time_ns=}"
            )
    ...
    async def enqueue_example(
        ...
        if shard.is_first_layer():
            loss = await self.process_example(
            ...
            if request_id is None:
                request_id = str(uuid.uuid4())
            self.outstanding_requests[request_id] = "waiting"
            loss = await self.forward_example(
                shard, example, target, length, train, request_id, 0
            )
        return loss
    ...
    async def coordinate_save(
        ...
        if (
            len(self.checkpoints[model][sid]) < 1
            or self.checkpoints[model][sid][-1] < iteration
        ):
            ...
            self.checkpoints[model][sid] = sorted(
    ...
    async def process_example(
        ...
        start_time = time.perf_counter_ns()
        resp = await self._process_example(
            shard, example, target, length, train, request_id
        )
        end_time = time.perf_counter_ns()
        elapsed_time_ns = end_time - start_time
    ...
    async def _process_example(
        ...
        shard = self.get_current_shard(base_shard)
        if DEBUG >= 1:
            print(f"[{request_id}] process_example: {example.shape=}")
        try:
            target = target.astype(int)
            if train:
                if shard.is_last_layer():
                    self.outstanding_requests[request_id] = "training"
                    loss, grad = await self.inference_engine.train(
                        request_id, shard, example, target, length
                    )
                else:
                    self.outstanding_requests[request_id] = "preprocessing"
                    step, _ = await self.inference_engine.infer_tensor(
                        request_id, shard, example
                    )
                    self.outstanding_requests[request_id] = "waiting"
                    loss, backgrad = await self.forward_example(
                        shard,
                        step,
                        target,
                        length,
                    ...
                    self.outstanding_requests[request_id] = "training"
                    partial_loss, grad = await self.inference_engine.train(
                        request_id,
                        shard,
                        example,
                        backgrad,
                ...
                if shard.is_last_layer():
                    ...
                    loss = await self.inference_engine.evaluate(
                        request_id, shard, example, target, length
                    )
                else:
                    self.outstanding_requests[request_id] = "preprocessing"
    ...
    async def forward_example(
        ...
        if DEBUG >= 1:
            print(f"target partition index: {target_index}")
        target_id = self.partitioning_strategy.partition(self.topology)[
            target_index
        ].node_id
        ...
        if DEBUG >= 1:
            print(
                f"sending example to {target_peer.id()}: {step} => {target} ({length})"
            )
        resp = await target_peer.send_example(
            target_shard, step, target, length, request_id=request_id, train=train
        )
        return resp
    ...
    async def forward_prompt(
        ...
        if target_id == self.id:
            ...
            if not target_peer:
                raise ValueError(f"Peer for {target_index} not found")
            if DEBUG >= 1:
                print(f"Sending prompt to {target_peer.id()}: {prompt}")
    ...
    async def forward_tensor(
        ...
        if target_id == self.id:
            ...
            if not target_peer:
                raise ValueError(f"Peer for {target_index} not found")
            if DEBUG >= 1:
                print(f"Sending tensor to {target_peer.id()}: {tensor}")
    ...
    def get_current_shard(
        self, base_shard: Shard, index: Optional[int] = None
    ) -> Shard:
        if index is None:
    ...
    async def select_best_inference_engine(self):
        ...
        if len(self.get_topology_inference_engines()):
            self.inference_engine = get_inference_engine(
                supported_engines[0], self.shard_downloader
            )
    ...
    def trigger_on_token_callbacks(
        ...
        if DEBUG >= 2:
            print(
                f"Triggering all on_token callbacks with {request_id=} {tokens=} {is_finished=}"
            )
    ...
    async def broadcast_result(
        ...
        if DEBUG >= 2:
            print(f"Broadcasting result: {request_id=} {result=} {is_finished=}")
