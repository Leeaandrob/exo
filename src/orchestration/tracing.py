...
class Tracer:
    ...
    def create_context_from_parent(
        self, request_id: str, trace_parent: str, sequence_number: int = 0
    ) -> TraceContext:
    ...
    def handle_token(
        self, context: TraceContext, token: int, is_finished: bool = False
    ):
        ...
        if context.token_group_span:
            ...
            if context.token_count % context.token_group_size == 0 or is_finished:
                context.token_group_span.set_attribute(
                    "token.final_count", relative_pos
                )
    ...
    def start_span(
        ...
        if parent_ctx:
            span = self.tracer.start_span(
                name, context=parent_ctx, attributes=attributes
            )
        else:
