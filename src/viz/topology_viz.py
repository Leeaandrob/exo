...
from helpers import exo_text, pretty_print_bytes, pretty_print_bytes_per_second
from topology.topology import Topology
from topology.partitioning_strategy import Partition
from download.download_progress import RepoProgressEvent
from topology.device_capabilities import UNKNOWN_DEVICE_CAPABILITIES
...
class TopologyViz:
    def __init__(
        self, chatgpt_api_endpoints: List[str] = [], web_chat_urls: List[str] = []
    ):
        ...
        self.main_panel = Panel(
            ...
            title="Tela Hypergrid (0 nodes)",
            border_style="bright_yellow",
        )
        self.prompt_output_panel = Panel(
            "", title="Prompt and Output", border_style="green"
        )
    ...
    def update_visualization(
        ...
        node_download_progress: Dict[str, RepoProgressEvent] = {},
    ):
        self.topology = topology
    ...
    def refresh(self):
        ...
        self.main_panel.title = (
            f"Tela Hypergrid ({node_count} node{'s' if node_count != 1 else ''})"
        )
    ...
    def _generate_prompt_output_layout(self) -> Panel:
        ...
        for prompt, output in reversed(requests):
            ...
            max_output_lines = (
                lines_per_request - max_prompt_lines - 1
            )  # -1 for spacing
    ...
    def _generate_main_layout(self) -> str:
        ...
        visualization = [
            [" " for _ in range(100)] for _ in range(48)
        ]  # Increased height to 48
        ...
        if len(self.web_chat_urls) > 0:
            info_lines.append(
                f"Web Chat URL (tinychat): {' '.join(self.web_chat_urls[:1])}"
            )
        if len(self.chatgpt_api_endpoints) > 0:
            info_lines.append(
                f"ChatGPT API endpoint: {' '.join(self.chatgpt_api_endpoints[:1])}"
            )
        ...
        visualization[bar_y - 1][
            bar_start_x + bar_width * 2 + 2 : bar_start_x + bar_width * 2 + 11
        ] = "GPU rich"
        ...
        for i, partition in enumerate(self.partitions):
            ...
            description1 = next(
                (
                    c.description
                    for c in conn1
                    if c.to_id == self.partitions[next_i].node_id
                ),
                "",
            )
            description2 = next(
                (c.description for c in conn2 if c.to_id == partition.node_id), ""
            )
    ...
    def _generate_download_layout(self) -> Table:
        ...
        if self.node_id in self.node_download_progress:
            ...
            for file_path, file_progress in download_progress.file_progress.items():
                if file_progress.status != "complete":
                    ...
                    percentage = (
                        f"{file_progress.downloaded / file_progress.total * 100:.0f}%"
                    )
        ...
        for node_id, progress in self.node_download_progress.items():
            if node_id != self.node_id:
                ...
                partition = next(
                    (p for p in self.partitions if p.node_id == node_id), None
                )
                partition_info = (
                    f"[{partition.start:.2f}-{partition.end:.2f}]" if partition else ""
                )
                percentage = (
