...
MODEL_NAME = "fanherodev/Llama-3.2-1B-Instruct"
...
def main(
    ...
    if not model.model.caches_are_enabled():
        with device:
            model.model.setup_caches(
                bsz, dtype, decoder_max_seq_len=total_response_length
            )
    ...
    if not padding_masks.all():
        ...
        mask = ttg.get_causal_mask_from_padding_mask(
    ...
    _, logits = model.generate(
        tokens=tokens, mask=mask, input_pos=input_pos, curr_pos=curr_pos
    )

    curr_pos = tokens_length

    q = torch.empty(
        (logits.size(0), model.model.tok_embeddings.num_embeddings),
        device=logits.device,
    ).exponential_(1, generator=rng)

    tokens = ttg.sample(
        logits=logits[:, -1].clone(), temperature=TEMP, top_k=TOP_K, q=q
    )
    ...
    for i in range(MAX_NEW_TOKENS - 1):
        ...
        _, logits = model.generate(
            tokens=tokens, input_pos=input_pos, mask=mask, curr_pos=curr_pos
        )
