import unittest
from src.models import get_supported_models, model_cards
from src.inference.inference_engine import inference_engine_classes
...
def expand_engine_lists(engine_lists):
    def map_engine(engine):
        return inference_engine_classes.get(
            engine, engine
        )  # Return original name if not found

    return [[map_engine(engine) for engine in sublist] for sublist in engine_lists]
