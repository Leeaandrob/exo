...
from helpers import AsyncCallbackSystem, AsyncCallback
...
async def main() -> None:
    ...
    async def wait_for_callback(
        name: str, callback: AsyncCallback[Any], condition: Callable[..., bool]
    ) -> None:
        try:
    ...
    await asyncio.gather(
        wait_for_callback(
            "Callback1",
            callback1,
            lambda msg, num, flag: isinstance(msg, str) and num > 0,
        ),
    ...
    await wait_for_callback(
        "NewCallback", new_callback, lambda msg, num, flag: num > 100
    )
