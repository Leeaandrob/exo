...
class DeviceFlops(BaseModel):
    """
    Represents the FLOPS (Floating Point Operations Per Second) capabilities of a device.

    Attributes:
        fp32 (float): FLOPS for FP32 operations in TFLOPS.
        fp16 (float): FLOPS for FP16 operations in TFLOPS.
        int8 (float): FLOPS for INT8 operations in TFLOPS.
    """
    ...
    def __str__(self):
        return (
            f"fp32: {self.fp32 / TFLOPS:.2f} TFLOPS, "
            f"fp16: {self.fp16 / TFLOPS:.2f} TFLOPS, "
            f"int8: {self.int8 / TFLOPS:.2f} TFLOPS"
        )
...
class DeviceCapabilities(BaseModel):
    """
    Represents the hardware capabilities of a device.

    Attributes:
        model (str): The model identifier of the device.
        chip (str): The chip name or identifier.
        memory (int): Total memory in MB.
        flops (DeviceFlops): The FLOPS capabilities of the device.
    """
    ...
    def __str__(self):
        return (
            ...
            f"Memory: {self.memory}MB. Flops: {self.flops}"
        )
...
CHIP_FLOPS = {
    ...
    # Intel CPUs
    "Intel(R) Xeon(R) Gold 5412U": DeviceFlops(
        fp32=1.4976 * TFLOPS,  # 1.4976 TFLOPS
        fp16=2.9952 * TFLOPS,  # Estimativa para FP16
        int8=5.9904 * TFLOPS,  # Estimativa para INT8
    ),
...
def find_flops(cpu_model: str) -> DeviceFlops:
    """
    Finds the FLOPS corresponding to the CPU model using partial matching.

    Args:
        cpu_model (str): The brand_raw string of the CPU.

    Returns:
        DeviceFlops: The FLOPS capabilities of the CPU. Returns zeroed FLOPS if not found.
    """
    for key in CHIP_FLOPS.keys():
        if key.lower() in cpu_model.lower():
            return CHIP_FLOPS[key]
    return UNKNOWN_DEVICE_CAPABILITIES.flops


async def device_capabilities() -> DeviceCapabilities:
    """
    Determines the device capabilities based on the operating system.

    Returns:
        DeviceCapabilities: The detected hardware capabilities of the device.
    """
    if psutil.MACOS:
...
async def mac_device_capabilities() -> DeviceCapabilities:
    """
    Detects device capabilities on macOS.

    Returns:
        DeviceCapabilities: The detected hardware capabilities of the macOS device.
    """
...
async def linux_device_capabilities() -> DeviceCapabilities:
    """
    Detects device capabilities on Linux.

    Returns:
        DeviceCapabilities: The detected hardware capabilities of the Linux device.
    """
    ...
    if Device.DEFAULT in ["CUDA", "NV", "GPU"]:
        ...
        return DeviceCapabilities(
            model=f"Linux Box ({gpu_name})",
            chip=gpu_name,
            memory=gpu_memory_info // 2**20,  # Adjust as necessary
            flops=CHIP_FLOPS.get(gpu_name, DeviceFlops(fp32=0, fp16=0, int8=0)),
        )
    else:
        # CPU Detection
        info = cpuinfo.get_cpu_info()
        cpu_model = info.get("brand_raw", "Unknown CPU").strip()

        # Get the maximum frequency using psutil
        freq_info = psutil.cpu_freq()
        max_freq = (
            freq_info.max if freq_info and freq_info.max else 3.9
        )  # Default to 3.9 GHz if unavailable

        # Update FLOPS calculation based on maximum frequency
        # Considering 24 physical cores and 16 FP32 operations per cycle
        num_cores = 24
        operations_per_cycle = 16
        flops_fp32 = num_cores * max_freq * 1e9 * operations_per_cycle / 1e12  # TFLOPS
        flops_fp16 = 2 * flops_fp32
        flops_int8 = 4 * flops_fp32

        # Dynamically update CHIP_FLOPS if CPU model is not found
        flops = find_flops(cpu_model)

        # If CPU is not in CHIP_FLOPS, use calculated FLOPS
        if flops.fp32 == 0 and flops.fp16 == 0 and flops.int8 == 0:
            flops = DeviceFlops(fp32=flops_fp32, fp16=flops_fp16, int8=flops_int8)

        return DeviceCapabilities(
            model=f"Linux Box ({cpu_model})",
            chip=cpu_model,
            memory=psutil.virtual_memory().total // 2**20,
            flops=flops,
        )
...
def windows_device_capabilities() -> DeviceCapabilities:
    """
    Detects device capabilities on Windows.

    Returns:
        DeviceCapabilities: The detected hardware capabilities of the Windows device.
    """
    import psutil
    ...
    def get_gpu_info():
        ...
        for gpu in gpus:
            info = {
                "Name": gpu.Name,
                "AdapterRAM": gpu.AdapterRAM,  # Bug in this property, returns negative for VRAM > 4GB (uint32 overflow)
    ...
    if contains_nvidia:
        ...
        # CPU Detection on Windows (no NVIDIA or AMD GPU)
        # Use py-cpuinfo to get CPU information
