    ...
    with open(model_config_path, "r") as f:
        ...
        model_config = {
            ...
            "torch_dtype": HF_PRECISION_STR_TO_DTYPE.get(
                base_config.get("torch_dtype", "float16"), torch.float16
            ),
        }

        if model_config.get("rope_scaling", None) is not None:
            model_config["rope_scaling_factor"] = model_config["rope_scaling"].get(
                "rope_factor", 32
            )
        ...
        if use_org_seq and model_config.get("rope_scaling", None) is not None:
            model_config["max_seq_len"] = model_config["rope_scaling"][
    ...
    for name, param in model_state_dict.items():
        if name in state_dict:
            ...
            if param.shape != loaded_param.shape:
                print(
                    f"Shape mismatch for {name}: expected {param.shape}, got {loaded_param.shape}"
                )
            else:
...
def load_model_weights_torchtune(
    ...
    for key, value in full_state_dict.items():
        ...
        for layer_num in range(shard.start_layer, shard.end_layer + 1):
            ...
            re_attn = re.findall(
                rf"model\.layers\.{layer_num}.(\w+)\.(\w+)\.(\w+)", key
            )
            if len(re_attn) != 0 and re_attn[0][0] == "self_attn":
                if re_attn[0][1] == "k_proj" and is_llama:
                    value = _permute(
                        t=value, n_heads=num_kv_heads, head_dim=head_dim, dim=dim
                    )

                    new_key = (
                        f"model.layers.{layer_num}.attn.{re_attn[0][1]}.{re_attn[0][2]}"
                    )
                    remapped_state_dict[new_key] = value
                elif re_attn[0][1] == "q_proj" and is_llama:
                    value = _permute(
                        t=value, n_heads=num_heads, head_dim=head_dim, dim=dim
                    )
                    new_key = (
                        f"model.layers.{layer_num}.attn.{re_attn[0][1]}.{re_attn[0][2]}"
                    )
    ...
    def forward(
        ...
        *,
        ...
        if hidden_state is not None:
            ...
            h = self.tok_embeddings(fl_tokens).to(
                dtype=dtype
            )  # Apply token tok_embeddings
