...
def find_available_port(
    ...
    while available_ports:
        ...
        if DEBUG >= 2:
            print(f"Trying to find available port {port=}")
        try:
...
class AsyncCallback(Generic[T]):
    ...
    async def wait(
        self, check_condition: Callable[..., bool], timeout: Optional[float] = None
    ) -> Tuple[T, ...]:
        async with self.condition:
            await asyncio.wait_for(
                self.condition.wait_for(
                    lambda: self.result is not None and check_condition(*self.result)
                ),
                timeout,
            )
    ...
    try:
        if NODE_ID_FILE.is_file():
            ...
            if is_valid_uuid(stored_id):
                if DEBUG >= 2:
                    print(f"Retrieved existing node ID: {stored_id}")
                return stored_id
            else:
                if DEBUG >= 2:
                    print("Stored ID is not a valid UUID. Generating a new one.")
        ...
        if DEBUG >= 2:
            print(f"Generated and stored new node ID: {new_id}")
        return new_id
    except IOError as e:
        if DEBUG >= 2:
            print(f"IO error creating node_id: {e}")
        return str(uuid.uuid4())
    except Exception as e:
        if DEBUG >= 2:
            print(f"Unexpected error creating node_id: {e}")
        return str(uuid.uuid4())
...
async def get_interface_priority_and_type(ifname: str) -> Tuple[int, str]:
    ...
    if ifname.startswith(
        ("tun", "tap", "vtun", "utun", "gif", "stf", "awdl", "llw", "wg0")
    ):
        return (1, "External Virtual")
...
def is_frozen():
    return (
        ...
        or os.path.basename(sys.executable) == "exo"
        or ("Contents/MacOS" in str(os.path.dirname(sys.executable)))
        or "__nuitka__" in globals()
...
def get_exo_home() -> Path:
    ...
    if not exo_folder.exists():
        exo_folder.mkdir(exist_ok=True)
    return exo_folder
